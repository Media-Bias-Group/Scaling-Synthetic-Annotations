...
For each task:
  1. Join the two teacher parquets on id, find disagreements.
  2. Build a BLIND prompt per disagreement: task definition + k=8 retrieved
     examples + target text. Teacher labels are NOT shown (no anchoring).
  3. Submit all disagreements as one Anthropic Message Batch.
  4. Poll until complete, download results, parse the final label.
  5. Write <task>_tiebreak.parquet (id, claude_label, claude_raw).
...
from stage4_config_news import (TASKS, EMBED_MODEL, INDEX_DIR, OUT_DIR,
                           K_SHOTS)
